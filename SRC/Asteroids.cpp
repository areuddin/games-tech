...
void Asteroids::OnKeyPressed(uchar key, int x, int y)
{
	...
	//view the instructions and prevent rules from showing when status 
	else if (key == 'r' && !gameStatus)
	{
...
// Create a new GUILabel and wrap it up in a shared_ptr
	mIntroLabel = shared_ptr<GUILabel>(new GUILabel("Press R to view game instructions."));
	// Set the horizontal alignment of the label to GUI_HALIGN_CENTER
	mIntroLabel->SetHorizontalAlignment(GUIComponent::GUI_HALIGN_CENTER);
	// Set the vertical alignment of the label to GUI_VALIGN_MIDDLE
	mIntroLabel->SetVerticalAlignment(GUIComponent::GUI_VALIGN_MIDDLE);
	// Set the visibility of the label to true (visible)
	mIntroLabel->SetVisible(true);
	// Add the GUILabel to the GUIContainer  
	shared_ptr<GUIComponent> intro_component
		= static_pointer_cast<GUIComponent>(mIntroLabel);
	mGameDisplay->GetContainer()->AddComponent(intro_component, GLVector2f(0.5f, 0.3f));
	...
	// Set the visibility of the label to false (hidden) initially
	mScoreLabel->SetVisible(false);  // Change this line
	...
	// Create a new GUILabel and wrap it up in a shared_ptr
	mRulesLabel = shared_ptr<GUILabel>(new GUILabel("Use ARROW keys to move and S to shoot"));
	// Set the horizontal alignment of the label to GUI_HALIGN_CENTER
	mRulesLabel->SetHorizontalAlignment(GUIComponent::GUI_HALIGN_CENTER);
	// Set the vertical alignment of the label to GUI_VALIGN_MIDDLE
	mRulesLabel->SetVerticalAlignment(GUIComponent::GUI_VALIGN_MIDDLE);
	// Set the visibility of the label to false (hidden)
	mRulesLabel->SetVisible(false);
	// Add the GUILabel to the GUIContainer  
	shared_ptr<GUIComponent> rules_component
		= static_pointer_cast<GUIComponent>(mRulesLabel);
	mGameDisplay->GetContainer()->AddComponent(rules_component, GLVector2f(0.5f, 0.5f));
